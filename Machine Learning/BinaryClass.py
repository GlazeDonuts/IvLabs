...
arr = pd.read_csv("temp.csv", sep=',', header=None,error_bad_lines=False)
print(arr)
arr = np.array(arr)
...
W, b = learn(W, b, X, m, Y, 10000, 0.001, activation="Sigmoid", show=True)
...
xval = np.array(np.linspace(np.min(X[1,:]),np.max(X[1,:]),100))
